...
翻译文章默认使用独立评论身份。英文文章保留 legacy identity；中文译文使用带语言作用域的 identity。Gitalk 在 Aurora 3 中仅用于迁移：切换 provider 前，应将保留的 UID/pathname 映射与历史记录对照；Aurora 3 不打包 Gitalk runtime。Valine、Twikoo 或 Waline 的连续性仍需核对对应服务商的外部记录。
