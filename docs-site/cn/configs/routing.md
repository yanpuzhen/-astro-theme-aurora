...
- UID 模式：设置 `ASTRO_PATH_SLUG=uid` 后使用 `/post/<legacyUid>/`
...
此时 `src/content/posts/demo-markdown-fundamentals.md` 的生成地址是 `/astro-theme-aurora/demo/post/demo-markdown-fundamentals/`。不要在内容或模板中假定站点总是在 `/`。
