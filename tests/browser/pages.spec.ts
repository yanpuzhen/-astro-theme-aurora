...
test('Demo pages remain readable without JavaScript', async ({ browser }) => {
  ...
  for (const path of ['/', '/cn/post/welcome-to-aurora-3-cn/', '/tags/', '/categories/', '/archives/']) {
...
test('Demo Chinese locale, translated article and locale-isolated search work', async ({ page }) => {
  ...
  await page.goto(demoRoute('/cn/post/welcome-to-aurora-3-cn/'))
  await expect(page.locator('.article-title')).toContainText('欢迎使用 Aurora 3.0')
  await expect(page.locator('link[rel="canonical"]')).toHaveAttribute('href', /\/astro-theme-aurora\/demo\/cn\/post\/welcome-to-aurora-3-cn\//)
  await expect(page.locator('link[hreflang="en"]')).toHaveAttribute('href', /\/astro-theme-aurora\/demo\/post\/welcome-to-aurora-3\//)
  await page.locator('link[hreflang="en"]').evaluate((element) => (element as HTMLLinkElement).href).then((href) => page.goto(href))
  await expect(page.locator('html')).toHaveAttribute('lang', 'en')
  await expect(page.locator('.article-title')).toContainText('Welcome to Aurora 3.0')
