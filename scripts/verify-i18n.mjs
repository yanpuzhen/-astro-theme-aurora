...
const englishFeed = readFileSync(`${root}/rss.xml`, 'utf8')
const chineseFeed = readFileSync(`${root}/cn/rss.xml`, 'utf8')
assert.doesNotMatch(sitemap, /architecture-smoke|legacy-compatibility|legacy-rich-markdown|unicode-torture/, 'test-only post fixtures must not enter the sitemap')
assert.doesNotMatch(englishFeed, /Architecture smoke post|Markdown Fundamentals in Aurora/)
assert.doesNotMatch(chineseFeed, /迁移后的第一篇文章/)
